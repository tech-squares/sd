/* 
 * sdui-mac.c - SD Macintosh User Interface
 *
...
long_boolean uims_show_verify = TRUE;
...
get_user_command(int which)
{
    ...
    for (;;) {
        ...
        if (c == '?') {
            match_begin();
            match_user_input(user_input, which, 0, 0, match_add, uims_show_verify);
            match_end();
            continue;
        }
