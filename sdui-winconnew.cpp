...
BOOL WINAPI control_c_handler(DWORD ControlInfo)
{
   ...
   if (ControlInfo == CTRL_C_EVENT) {
      ...
      inputRecord.Event.KeyEvent.wVirtualScanCode = 46;
      inputRecord.Event.KeyEvent.uChar.AsciiChar = 'z';
...
extern int get_char()
{
   ...
   for ( ;; ) {
      ...
      else if (inputRecord.EventType == MENU_EVENT && inputRecord.Event.KeyEvent.bKeyDown) {
         unsigned int c = inputRecord.Event.MenuEvent.dwCommandId;
         if (c == 0x0116)
            numRead++;
         else if (c == 0x011F)
            numRead++;
